/** ChatSidebar: conversation history list with date grouping, inline rename, and a new-conversation button. */
...
// Format a date string as "Hôm nay", "Hôm qua", or a locale date string.
...
// Return sort priority for date group labels (Today = 0, Yesterday = 1, all others = 2).
...
// Sidebar listing conversations grouped by date, with rename/delete dropdown and new-chat button.
export function ChatSidebar({
  ...
  // Begin inline title editing for a conversation item.
  const handleStartEdit = (conv: Conversation) => {
    setEditingId(conv.id);
    setEditTitle(conv.title);
  };

  // Commit the edited title (if non-empty) and exit edit mode.
  ...
  // Group conversations by formatted date label.
  ...
  // Order groups: Hôm nay → Hôm qua → rest (newest group first).
  ...
  // Resolve display name from whichever user field is available.
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2 pb-4 scrollbar-thin">
        {conversations.length === 0 && (
          <p className="px-3 py-6 text-center text-xs text-muted-foreground">
            ...
            <br />
            Nhấn <span className="font-medium text-foreground">+</span> để bắt đầu.
          </p>
        )}
                ...
                <li key={conv.id}>
                  {editingId === conv.id ? (
                    <input
                      ...
                      className="w-full rounded-lg border border-gray-300 bg-background px-3 py-2 text-sm ring-1 ring-gray-300/60 outline-none"
                      autoFocus
                    />
                  ) : (
                    <div
                    ...
                    >
                      <MessageSquare className={cn(
                        ...
                        activeConversationId === conv.id ? "text-gray-900" : "text-muted-foreground",
                      )} />
